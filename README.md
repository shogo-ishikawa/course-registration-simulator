...
**現在、9学科ともクラス対応の自動判定は準備中です。** 公式リンクは確認済みですが、
Boxのクラス分け表の本文はこの更新時に取得できていません。学籍番号とクラスの対応は
推測していません。`src/data/department-guidance.json` の `verifiedAt` は `null`、
`rules` は空配列としており、画面にも準備中と表示します。

公式表を読めるようになったら、科目ごとに学籍番号の範囲、クラス名、講義コード、
科目キー、開講Q、担当教員、曜日時限を実際の時間割と照合して `rules` へ追加し、
本文の確認日を `verifiedAt` に記録します。形式は `src/class-guidance.ts` の
`ClassRule` を参照してください。氏名・成績・原本ファイルは公開リポジトリへ追加しません。
個別名簿しかない場合は、公開可能な規則へ整理できるかを先に確認します。

自動判定は対象年度・学年と学籍番号全体を照合し、学期内で一意に対応する科目だけを返します。
複数候補、古い講義コード、担当教員や時間の変更を検出した科目は手動確認に戻します。
